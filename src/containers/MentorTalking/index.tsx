...
export default function MentorTalking() {
  ...
  const onChange = (key: any, value: any) => {
    const newListAnswer: any = [...listAnswer];
    const validItem = newListAnswer.findIndex((x: any) => x.key === key);
    if (validItem !== -1) {
  ...
  const getItemChecked = (index: any) => {
    const newListAnswer = [...listAnswer];
    const validCheck = newListAnswer.findIndex((x: any) => x.key === index);
    if (validCheck !== -1) {
